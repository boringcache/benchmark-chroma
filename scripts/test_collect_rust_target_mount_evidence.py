...
class CollectRustTargetMountEvidenceTest(unittest.TestCase):
    def test_reports_rolling_target_archive_growth(self) -> None:
        log = """
#8 boringcache cache mount hydrate cacheID="/chroma-cargo-target" status=hit archive="target-old" compressedBytes=10000000 transfer=stream total=1s
#19 boringcache cache mount publish cacheID="/chroma-cargo-target" status=archive_built compressedBytes=10000061 uncompressedBytes=50000000 files=120 archive=1s
#19 boringcache cache mount publish cacheID="/chroma-cargo-target" status=published compressedBytes=10000061 total=1s
"""
        payload = summarize(
            parse_events(log, "^/?chroma-cargo-target$"),
            "^/?chroma-cargo-target$",
        )
    ...
    def test_uses_terminal_publish_events_from_managed_trace(self) -> None:
        log = '''
#8 boringcache cache mount hydrate cacheID="/chroma-cargo-target" status=miss archive="target-new" http_status=404
'''
        trace = '''
{"operation":"cache_session_summary","buildkit":{"mountcache":{"samples":[{"event":"mountcache_publish_archive_built","cache_id":"/chroma-cargo-target","compressed_bytes":739526718,"uncompressed_bytes":3464012613,"file_count":8915},{"event":"mountcache_publish_done","cache_id":"/chroma-cargo-target","compressed_bytes":739526718,"uncompressed_bytes":3464012613,"file_count":8915}]}}}
'''
        pattern = "^/?chroma-cargo-target$"
        payload = summarize(
            merge_events(
...
#8 boringcache cache mount hydrate cacheID="/chroma-cargo-target" status=miss archive="target-new" http_status=404
'''
        events = parse_events(log, "^/?chroma-cargo-target$")

        with self.assertRaisesRegex(ValueError, "did not complete"):
            summarize(events, "^/?chroma-cargo-target$")
